...
McNemar's test is used for determining whether the performance differences between the models (tiny, medium and large-v3) are statistically significant. The models are compared pairwise, and the test counts the number of instances where one model produces an error and the other does not. Based on these counts, the test produces chi-squared and p-values. The statistical significance is determined as p < 0.05 meaning that the differences are unlikely to be caused by random variations.